...
export class Skip implements Command {
    ...
    execute(msg: Message, args: string[]): void {
        ...
        if (!msg.member.voiceChannel) msg.channel.send('You have to be in a voice channel to stop the music!');
        else if (!serverEntry) msg.channel.send('There is no song that I could skip!');
        else if (args[0]) {
            if (Number(args[0])) {
