...
export class Spotify {
  ...
  getSong(url: string) {
    return new Promise<Song | null>(async (res, rej) => {
      ...
      Request.get(SPOTIFY_API_URI + 'tracks/' + songId, {headers: {Authorization: 'Bearer ' + SPOTIFY.SPOTIFY_ACCESS_TOKEN, 'Content-type': 'application/json'}}, async (error, response, body) => {
        const searchString = JSON.parse(body).album.artists[0].name + '+' + JSON.parse(body).name
